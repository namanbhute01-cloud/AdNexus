...
let CampaignsService = class CampaignsService {
    ...
    bucket;
    cdnDomain;
    constructor(campaignsRepo, configService) {
    ...
    async uploadCampaign(file, metadata) {
        if (!file) {
            throw new common_1.NotFoundException('Missing campaign file');
        }
        const id = (0, crypto_1.randomUUID)();
        const ext = path.extname(file.originalname) || '.bin';
        const key = `campaigns/${id}/asset${ext}`;
        const checksum = `sha256:${(0, crypto_1.createHash)('sha256').update(file.buffer).digest('hex')}`;
    ...
    async getDownloadUrl(campaignId) {
    ...
    async validateChecksum(campaignId, hash) {
        const campaign = await this.campaignsRepo.findOne({ where: { id: campaignId } });
        if (!campaign) {
            throw new common_1.NotFoundException(`Campaign ${campaignId} not found`);
        }
    ...
    async generateThumbnail(buffer, campaignId) {
