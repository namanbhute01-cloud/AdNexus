...
let SchedulesService = class SchedulesService {
    ...
    async validateSyncScope(screenIds, mode, deviceId) {
        if (mode === schedule_entity_1.PlayMode.INDEPENDENT)
            return;
        const screens = await this.screensRepo.findByIds(screenIds);
        const evIds = new Set(screens.map(s => s.deviceId));
        if (evIds.size > 1) {
            throw new common_1.BadRequestException('MIRROR/COMBINED mode requires all screens on the same EV unit. ' +
                'These screens belong to different EVs.');
        }
    }
    async checkOverlap(deviceId, screenPosition, start, end, excludeId) {
        ...
        if (screenPosition === schedule_entity_1.ScheduleScreenPosition.ALL) {
            queryBuilder.andWhere('s.screen_position IN (:...positions)', {
                positions: [schedule_entity_1.ScheduleScreenPosition.A, schedule_entity_1.ScheduleScreenPosition.B, schedule_entity_1.ScheduleScreenPosition.C],
            });
        }
        else {
        ...
        if (overlapping.length > 0) {
            throw new common_1.ConflictException(`Schedule overlaps with existing slot(s): ${overlapping.map(s => s.id).join(', ')}`);
        }
    }
